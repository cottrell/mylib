...
from .tools import AttrDict, TimeLogger
...
def from_dict_of_things(path):
    ...
    funs = {"pickle": _from_pickle, "carrays": from_carrays, "block": lambda x: _from_block(x)[:]}  # return numpy to avoid confusion
    d = dict()
    for i, k in enumerate(meta["keys"]):
        t = meta["types"][k]
...
def to_carrays(df, path, format_categories=["bcolz"], format_codes=["bcolz"], format_values=["bcolz"]):
    ...
    for i, k in enumerate(df):
        _to_carray(df[k], k, os.path.join(path, str(i)), format_categories=format_categories, format_codes=format_codes, format_values=format_values)  # , cparams=bcolz.cparams(clevel=9, shuffle=True, cname='blosclz'))
